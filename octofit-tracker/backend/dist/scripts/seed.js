...
async function seedDatabase() {
    ...
    const teams = await team_model_1.default.insertMany([
        { name: 'Octo Sprinters', city: 'Seattle', memberIds: [users[0]._id, users[1]._id] },
        { name: 'Core Climbers', city: 'Austin', memberIds: [users[2]._id] }
    ]);
    await user_model_1.default.updateOne({ _id: users[0]._id }, { teamId: teams[0]._id });
    await user_model_1.default.updateOne({ _id: users[1]._id }, { teamId: teams[0]._id });
    await user_model_1.default.updateOne({ _id: users[2]._id }, { teamId: teams[1]._id });
    await activity_model_1.default.insertMany([
        {
            userId: users[0]._id,
    ...
    await leaderboard_model_1.default.insertMany([
